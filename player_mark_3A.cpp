...
const std::array<Point, 4> corner{{//four corner
    Point(0, 0), Point(0, 8),
    Point(8, 0), Point(8, 8)
}};
const std::array<Point, 12> danger_spots{{
    /*upleft*/ Point(0, 1), /*upupmid*/ Point(0,7),/*upright*/
    Point(1, 0), Point(1, 1), /*upmid*/ Point(1, 7), Point(1, 8),
    ///////////////*the middle of the board*///////////////////
    Point(7, 0), Point(7, 1), /*downmid*/ Point(7, 7), Point(7, 8),
    /*downleft*/ Point(8, 1), /*downdownmid*/ Point(8, 7) /*downright*/
}};
...
void find_spot_value(Board b, int player){
    for(Point vp: next_valid_spots){
        ...
        int board_value = alphabeta(next, 1, 3 - player);
...
void write_valid_spot(std::ofstream& fout) {
    ...
    if(!next_valid_spots.size()) return; //no valid spot
    //find the max path //add the strategy
    int index = 0;
    int value = -9999;
    for(int i = 0; i < n_valid_spots; i++){
        if(next_valid_spots[i] == corner[0] ||
        next_valid_spots[i] == corner[1] ||
        next_valid_spots[i] == corner[2] ||
        next_valid_spots[i] == corner[3]){
            index = i;
            break;
        }
        if(spots_value[i] >= value){
            if(next_valid_spots[i] == danger_spots[0] || 
            next_valid_spots[i] == danger_spots[1] || 
            next_valid_spots[i] == danger_spots[2] || 
            next_valid_spots[i] == danger_spots[3] || 
            next_valid_spots[i] == danger_spots[4] || 
            next_valid_spots[i] == danger_spots[5] || 
            next_valid_spots[i] == danger_spots[6] || 
            next_valid_spots[i] == danger_spots[7] || 
            next_valid_spots[i] == danger_spots[8] ||
            next_valid_spots[i] == danger_spots[9] ||
            next_valid_spots[i] == danger_spots[10] ||
            next_valid_spots[i] == danger_spots[11] || i != n_valid_spots-1)
                continue;
            value = spots_value[i];
            index = i;
        }
    ...
    if(!is_spot_valid(board, p, player)){//prevent invalid move
        ...
        while(!is_spot_valid(board, p, player) && i < n_valid_spots){
            i++;
            p = next_valid_spots[i];
        }
    }
