...
int alphabeta(Board board, int depth, int cur_player, int alpha = -INF, int beta = INF){
    int value;
...
Point find_spot_value(std::ofstream& fout){
    Point best;
    int best_value = -INF;
    int d = 5;
    /*
    ...
    if(next_valid_spots.size() >= 9)
        d--;
    */
    for(Point vp: next_valid_spots){
        Board next = predict_board(board, vp, player);
        int board_value = alphabeta(next, d, opposite_player);
